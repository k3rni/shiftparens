...
class UInputEmitter:
    ...
    KEYSEQ = {
        '(': [(KEY_LEFTSHIFT, 1), (KEY_9, 1), (KEY_9, 0), (KEY_LEFTSHIFT, 0)],
        ')': [(KEY_LEFTSHIFT, 1), (KEY_0, 1), (KEY_0, 0), (KEY_LEFTSHIFT, 0)],
        "\033": [(KEY_ESC, 1), ('syn', 0), (KEY_ESC, 0)]
    }

    def __init__(self, quiet=False):
        self.quiet = quiet
        self.log("Using UInput emitter")
        ...
        dev.expose_event(EV_KEY, KEY_LEFTSHIFT)
    ...
    def type(self, keys):
        for chr in keys:
            for (ev_code, ev_value) in self.KEYSEQ[chr]:
                tmval = timeval()
                self.libc.gettimeofday(byref(tmval), None)
                self.log("%d:%d %s %s %d" % (tmval.tv_sec, tmval.tv_usec, rev_events[EV_KEY], rev_event_keys[EV_KEY][ev_code], ev_value))
